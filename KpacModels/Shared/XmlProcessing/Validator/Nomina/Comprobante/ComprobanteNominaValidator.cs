...
namespace KpacModels.Shared.XmlProcessing.Validator.Nomina.Comprobante;

public class ComprobanteNominaValidator
{
    private readonly ValidatorContext _context;

    public ComprobanteNominaValidator(ValidatorContext context)
    {
        _context = context;
    }

    public void Validate(Comprobante40 comprobante)
    {
        ...
        // ValidateMoneda(comprobante.Moneda);
    ...
    private void ValidateFecha(string? fecha)
    {
        ...
        if (!RegexCatalog.IsIso8601Valid(fecha))
        {
            _context.AddError(
                code: "CFDI40101",
    ...
    private void ValidateFormaPago(string? formaPago)
    {
        if (!string.IsNullOrWhiteSpace(formaPago))
        {
            _context.AddWarning(
                section: "Comprobante",
                message: "El campo FormaPago no debe existir");
            return;
        }
    }
