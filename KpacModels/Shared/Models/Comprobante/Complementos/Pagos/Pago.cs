...
public class Pago
{
    ...
    public async Task Accept(IVisitorPagos visitor, int numPago)
    {
        ...
        for (int i = 0; i < countDr; i++)
        {
            ...
            await documentoRelacionado.Accept(visitor, numPago: numPago, numDocto: i + 1);
        }

        if (visitor.HasErrors()) return;
        visitor.Visit(decimal.Parse(this.Monto), numPago);
        if (visitor.HasErrors()) return;
        Impuestos?.Accept(visitor, numPago: numPago);
    }
